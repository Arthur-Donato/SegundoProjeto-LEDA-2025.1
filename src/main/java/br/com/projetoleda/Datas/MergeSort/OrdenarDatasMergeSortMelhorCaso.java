package br.com.projetoleda.Datas.MergeSort;
...
public class OrdenarDatasMergeSortMelhorCaso { 
    private static final String caminhoArquivoParaSerLido = "./Dados/games_release_date_MergeSort_medioCaso.csv";
    private static final String CAMINHO_ARQUIVO_GERADO = "./Dados/games_release_date_MergeSort_melhorCaso.csv";
    ...
    public static void merge(CSVRecord[] lista, int primeiroIndice, int indiceDoMeio, int ultimoIndice) {
        ...
        while(indiceArrayLeft < tamanhoArrayLeft && indiceArrayRight < tamanhoArrayRight){
            Date dataArrayEsquerda = extrairValor(arrayLeft, indiceArrayLeft);
            Date dataArrayDireita = extrairValor(arrayRight, indiceArrayRight);

            
            if(dataArrayEsquerda.compareTo(dataArrayDireita) <= 0){ 
